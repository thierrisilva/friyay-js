...
(function ($) {
	...
	$.tutorialize.obj = {
		...
		getArrowPosition: function(){
			...
			if (this.currentSlide.arrowIndex > 0) {
				// TIPHIVE
				css["background-position"] = '0 -' + (this.currentSlide.arrowSize*this.currentSlide.arrowIndex) + 'px';
			};
		...
		getSlideDefaultValues: function(slide){
			...
			slide.hOffset = slide.hOffset ? slide.hOffset : this.options.hOffset;  // TIPHIVE
			slide.vOffset = slide.vOffset ? slide.vOffset : this.options.vOffset; // TIPHIVE
			slide.cssPosition = slide.cssPosition ? slide.cssPosition : this.options.cssPosition; //TIPHIVE
			slide.arrowPath = slide.arrowPath ? slide.arrowPath : this.options.arrowPath;
			slide.arrowIndex = slide.arrowIndex ? slide.arrowIndex : this.options.arrowIndex;  // TIPHIVE
		...
		getSlidePosition: function($selector, $slide){
			...
			returnData = {
				// TIPHIVE - added Offsets
				left: slideLeft + this.currentSlide.hOffset,
				slidePosition: slidePosition,
				top: slideTop + this.currentSlide.vOffset
			}
		...
		nextSlide: function(slideIdx){
			...
			if ($.isFunction(this.options.onNext)) this.options.onNext.apply(this, [this.currentSlideIdx, this.currentSlide, this.$tutorializeElements.find('.tutorialize-slide')]);
		},
		...
		runSlide: function(){
			...
			$slide.css({
				...
				position: ($slideSelector.css('position') === 'fixed' || (this.currentSlide.position === 'center-center' && this.currentSlide.selector === 'html') ) ? 'fixed' : this.currentSlide.cssPosition,
		...
		runSlide_BuildControls: function(){
			...
			var slidesLength = this.options.slides.length,
					control = '<ul class="tutorialize-slide-control">' +
										// TIPHIVE - added counter options
										(this.options.showCounter ? '<li class="counter">'+(this.currentSlideIdx+1)+'/'+slidesLength+'</li>' : '') +
